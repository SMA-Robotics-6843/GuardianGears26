...
public class Arena2026Rebuilt extends SimulatedArena {
  ...
  protected DoublePublisher phaseClockPublisher = genericInfoTable.getDoubleTopic("Time left in current phase")
      .publish();
   ...
   *
   * <p>
   * This will create an Arena with the ramp areas marked as inaccessible. If you
   * would like to change that use
   * {@link #Arena2026Rebuilt(boolean)}. Additionally due to performance issues
   * the arena will not spawn all fuel by
   * default. If you would like to change this use
   ...
   *
   * <p>
   * Due to the nature of maple sim they can not be fully simulated and so either
   * must be non existent or treated
   * as full colliders. This behavior can be changed with the AddRampCollider
   * variable. Additionally due to
   * performance issues the arena will not spawn all fuel by default. If you would
   * like to change this use
   * {@link #setEfficiencyMode(boolean)}
   *
  ...
  public static Pose3d flip(Pose3d toFlip) {
    return new Pose3d(
        new Translation3d(RebuiltFieldObstaclesMap.FIELD_WIDTH - toFlip.getX(),
   ...
   *
   *
   * <h2>Generates a random number within a range centered on 0 with a variance
   * set by the parameter.</h2>
   *
   ...
   *
   * This method is useful for certain spawners like the return cutes on the hub
   * to prevent the game pieces from being
   ...
   * {@link #setShouldRunClock(boolean)}) is
   * false.
   *
   ...
   *
   * When this is set too false both goals will always be set to active. Ths can
   * be useful for testing or too simulate
   * endgame.
   *
   ...
   *
   *
   * <h2>Throws a game piece from the outpost at the specified angle and
   * speed.</h2>
   *
   * <p>
   * This method comes with variance built in (to simulate human inconsistency).
   ...
   * performance. MapleSim was not
   * designed with 400 game pieces in mind and so can struggle with the large
   * number of game pieces present in
   * rebuilt.
   *
